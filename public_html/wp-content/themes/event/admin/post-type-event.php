...
function themify_theme_event_meta_box( $args = array() ) {
	...
	return array(
		...
		array(
			...
			'type' => 'separator',
			'meta' => array(
				'html' => '<h4>' . __( 'Top Featured Area in Single Post', 'themify' ) . '</h4><hr class="meta_fields_separator"/>'
			),
		),
		// Background Color
		array(
...
if ( ! class_exists( 'Themify_Event' ) ) {
	...
	class Themify_Event {
		 ...
		 */
		function register() {
			register_post_type( $this->post_type, apply_filters(
				'themify_theme_register_' . $this->post_type . '_post_type',
				array(
					'labels' => array(
		...
		function type_column_header( $columns ) {
			...
			// $columns['date'] = __( 'Published Date', 'themify' );
			...
			if ( ! isset( $atts['image_w'] ) || '' == $atts['image_w'] ) {
				...
				switch ( $atts['style'] ) {
					case 'grid4':
						...
						$this->atts['image_h'] = 195;
						break;
					case 'grid3':
					case '':
						$this->atts['image_w'] = 360;
						$this->atts['image_h'] = 270;
						break;
					case 'grid2':
						$this->atts['image_w'] = 560;
						$this->atts['image_h'] = 420;
						break;
					case 'slider':
		 ...
		 * @return string|void
		 */
		function shortcode( $atts = array(), $post_type, $original_atts ) {
			global $themify;
			extract( $atts );
			...
			/* unless explicitly specified, show the upcoming events by default */
			if( $event_tab == 'no' && ( ! isset( $original_atts['past_events'] ) ) ) {
				$past_events = 'no';
			}
			...
			if( $upcoming_events == 'yes' ) {
				/* get events that either don't have end date or the date is not passed yet */
				...
				// Get upcoming events
				$events['upcoming'] = get_posts( apply_filters( 'themify_'.$post_type.'_shortcode_args', $args, $atts ) );
			}
			...
			if ( 'yes' == $past_events ) {
				...
				$temp_order = $themify->order;
				$themify->order = $order;
				$events['past'] = get_posts( apply_filters( 'themify_'.$post_type.'_shortcode_args', $args, $atts ) );
				$themify->order = $temp_order;
			}
		...
		function get_map( $address ) {
			if( ! function_exists( 'themify_shortcode' ) )
				include_once( trailingslashit( THEMIFY_DIR ) . 'themify-shortcodes.php' );
		...
		public function data_update_notice() {
			...
			if ( get_option( 'theme_event_data_update_154_done' ) != 'yes' ) {
		...
		public function do_data_updater() {
			if ( isset( $_GET['theme_event_update_datetime'] ) && true == $_GET['theme_event_update_datetime'] && get_option( 'theme_event_data_update_154_done' ) != 'yes' ) {
				...
				if ( $events ) {
					...
					update_option( 'theme_event_data_update_154_done', 'yes' );
				}
	...
	function widget( $args, $instance ) {
		...
		foreach( $events as $posts ) {
			if ( $posts ) {
				...
				$themify->unlink_title = 'no';
				$themify->unlink_image = 'no';
				...
				$themify->event_tab = !$hide_past_events ? 'yes' : 'no';
				...
				$btabs .= '
					<div class="shortcode ' . $post_type  . '-widget '. $themify->post_layout .'">' .
						themify_get_shortcode_template($posts, 'includes/loop-' . $post_type, 'widget') .
					'</div>
				</li>';

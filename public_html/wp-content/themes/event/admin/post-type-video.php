		...
		function shortcode($atts = array(), $post_type) {
			...
			if ($posts) {
				...
				if ( '' != $this->thumbnails && $is_slider ) {
					$out .= '<div id="' . $video_loop_id . '-thumbs" class="video-pager"><div class="slideshow-wrap"><div class="slideshow"  data-wrap="yes" data-pager="no" data-slidernav="no" >'.$this->thumbnails.'</div></div></div>';
				}
			...
			if ( '' != themify_get( 'video_url' ) ) {
				...
				if ( current_theme_supports( 'themify-post-in-lightbox' ) ) {
					if ( ! is_single() && '' != themify_get( 'setting-open_inline' ) ) {
						$link = get_permalink() . '?post_in_lightbox=1" class="themify_lightbox';
					}
